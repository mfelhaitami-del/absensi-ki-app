...
# PASTIKAN MASUKKAN URL /exec HASIL NEW DEPLOYMENT
WEBAPP_URL = "https://script.google.com/macros/s/AKfycbxAo7KJjJKmmCj60Xdt9szcX1IFtgAJtcLxCqMftVY_-eNLmEf_HRherNoYz4xqlgfG/exec"
    ...
    if st.button("🔍 Tampilkan Data Rekap", use_container_width=True):
        try:
            ...
            if data_json:
                ...
                # Menampilkan tabel statis agar format string tidak berubah
                st.table(df[['No', 'Nama', 'Tanggal', 'Jam Masuk', 'Jam Pulang']])
            else:
                st.info(f"Belum ada data rekap.")
        except Exception as e:
            st.error(f"Gagal memuat data rekap.")
