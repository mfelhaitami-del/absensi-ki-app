...
# --- KONFIGURASI ---
# Masukkan API Key ImgBB dan URL Web App Google Script Anda di sini
...
def halaman_presensi(waktu_aktif, status_absen, tgl_skrg):
    ...
    if status_absen == "TUTUP":
        st.error(f"🚫 Sesi absensi tutup. (Sekarang: {waktu_aktif.strftime('%H:%M:%S')})")
    else:
        # DAFTAR NAMA (Pastikan tanda kutip lengkap)
