...
WEBAPP_URL = "https://script.google.com/macros/s/AKfycbz8mdMXBGsHqbgsYr9zceGOl2u3TAlj4A4ayZM0f2ewjNm9Z0G9gE1LtOgrY3QJhFYJ/exec"
...
def process_watermark(foto, nama, w_skrg):
    ...
    img = Image.fromarray(np.flip(np.array(img), axis=1)) # Fix Mirroring
    draw = ImageDraw.Draw(img)
    
    # Text info
    txt = f"NAMA: {nama}\nWAKTU: {w_skrg.strftime('%d/%m/%Y %H:%M:%S')}\nVERIFIKASI: TIM KI"
    pos = (20, img.height - 80)
    
    # Tambah bayangan teks
            ...
            if foto:
                with st.spinner("Sedang memproses..."):
                    ...
                    if kirim_data(nama, sesi, foto_final, w_skrg):
                        st.success(f"✅ Berhasil! Selamat bekerja/pulang, {nama}.")
                        time.sleep(2)
                        st.rerun()
                    else:
                        st.error("❌ Gagal mengirim data. Cek koneksi Anda.")
            else:
...
# Halaman Rekap
    ...
    b = st.selectbox("Bulan:", list_b, index=w_skrg.month - 1)
    
    if st.button("🔍 Tampilkan Data", use_container_width=True):
        try:
            res = requests.get(f"{WEBAPP_URL}?bulan={b} {w_skrg.year}", timeout=20).json()
            if res:
                df = pd.DataFrame(res)
                st.dataframe(df, use_container_width=True, hide_index=True)
            else:
                st.info("Data bulan ini belum ada.")
        except:
